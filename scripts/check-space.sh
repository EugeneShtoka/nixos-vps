...
# Requires: MATRIX_SERVER, MATRIX_ACCESS_TOKEN, MATRIX_CUSTOM_SPACES env vars.
...
PARENTS=$(curl -sf \
  ...
  "$MATRIX_SERVER/_matrix/client/v3/rooms/$ROOM/state" \
...
for SPACE_ID in $PARENTS; do
  NAME=$(curl -sf \
    -H "Authorization: Bearer $MATRIX_ACCESS_TOKEN" \
    "$MATRIX_SERVER/_matrix/client/v3/rooms/$SPACE_ID/state/m.room.name" \
    | jq -r '.name // empty')
  [ -z "$NAME" ] && continue
